...
result_logger = setup_logger("search_result", "search_results.log")
...
class SearchTool(BaseArtifactTool):
    ...
    def _get_search_params_schema(self) -> Dict[str, types.Schema]:
        ...
        return {
            "query": types.Schema(
                type=types.Type.STRING, description="The search query string."
            ),
            "limit": types.Schema(
    ...
    async def _execute_search(
        ...
        try:
            ...
            logger.warning(
                "Could not retrieve user_id from tool_context. Using default 'unknown'."
            )
            user_id = "unknown"
