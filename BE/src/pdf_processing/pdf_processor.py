...
class PDFProcessor:
    ...
    IMAGE_CLASSES = {"Picture", "Table", "Figure"}
    ...
    def __init__(
        ...
        self.batch_size = batch_size or pdf_config.get("batch_size", 5)
    ...
    def process(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Processing complete! Chapters: {len(chapters)}, Images: {self.image_counter}"
        )
        logger.info(
            f"Results saved to S3: s3://{self.s3_client.bucket_name}/{self.s3_prefix}/"
        )
    ...
    def _detect_objects(self, page_images: List[Path]) -> Dict[str, List[Dict]]:
        ...
        logger.info(
            f"Running YOLO prediction on {len(page_images)} pages with batch_size={self.batch_size}"
        )

        # Process in batches
        for i in range(0, len(page_images), self.batch_size):
            batch_paths = page_images[i : i + self.batch_size]
            logger.debug(
                f"Processing batch {i//self.batch_size + 1}: {len(batch_paths)} images"
            )
            ...
            _ = self.model.predict(
                ...
                name="predict",
                exist_ok=True,  # Allow appending to existing directory
                verbose=False,
            )
        ...
        logger.info(
            f"Detected {total_detections} objects across {len(page_images)} pages"
        )
        return detections
    ...
    def _dedup_detections(
        self, dets: List[Dict], iou_thresh: float = 0.90
    ) -> List[Dict]:
    ...
    def _extract_content(
        self, page_images: List[Path], detections: Dict[str, List[Dict]]
    ) -> List[str]:
    ...
    def _extract_text_from_bbox(
        self, pdf_page: Any, bbox_xyxy: List[int], img_w: int, img_h: int
    ) -> str:
    ...
    def _save_results(self, chapters: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for img_file in tqdm(
            list(self.images_dir.glob("*.png")), desc="Uploading images"
        ):
            s3_key = f"{self.user_id}/{img_file.name}"
            self.s3_client.upload_file(str(img_file), s3_key)

        logger.info(
            f"All files uploaded to S3: s3://{self.s3_client.bucket_name}/{self.user_id}/"
        )
        return result
