...
from ..embedding_services.api import get_document_indexer
...
async def upload_and_process_pdf(
    ...
    try:
        ...
        processor = PDFProcessor(pdf_path=str(pdf_path), user_id=user_id, config=config)
        ...
        if config.get("indexing", {}).get("auto_index_on_upload", True):
            try:
                ...
                if background_tasks:
                    ...
                    logger.info(
                        f"Added background task to index book: {result['book_name']}"
                    )
                else:
                    # If no background tasks object (shouldn't happen with FastAPI), run sync?
                    # Better to just log warning, but for now let's assume it exists.
                    logger.warning("No background_tasks object available for indexing")
            except Exception as e:
                logger.error(f"Failed to schedule auto-indexing: {str(e)}")
...
async def get_processing_status(book_name: str, user_id: str):
    ...
    if not s3_client.object_exists(json_s3_key):
        raise HTTPException(
            status_code=404, detail=f"Book '{book_name}' not found in S3"
        )
...
async def get_chapter_content(book_name: str, chapter_id: int, user_id: str):
    ...
    # Find chapter
    chapter = next(
        (ch for ch in data["chapters"] if ch["chapter_id"] == chapter_id), None
    )
...
async def delete_processed_book(book_name: str, user_id: str):
    ...
    try:
        ...
        logger.info(
            f"Deleted book '{book_name}' and associated images from S3 for user '{user_id}'"
        )
