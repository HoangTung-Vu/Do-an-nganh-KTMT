...
class VectorStoreClient:
    ...
    def upsert_points(
        self, collection_name: str, points: List[Dict[str, Any]], batch_size: int = 100
    ) -> int:
        """
        ...
        try:
            ...
            for i in range(0, len(point_structs), batch_size):
                ...
                logger.debug(
                    f"Upserted batch {i//batch_size + 1}/{(len(point_structs)-1)//batch_size + 1}"
                )
    ...
    def delete_by_filter(
        ...
        try:
            ...
            logger.info(
                f"Deleted points from '{collection_name}' matching filter: {filter_conditions}"
            )
