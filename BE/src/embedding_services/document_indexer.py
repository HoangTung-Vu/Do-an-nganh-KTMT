...
class DocumentIndexer:
    ...
    def __init__(
        ...
        logger.info(
            f"Document indexer initialized with S3: s3://{self.s3_client.bucket_name}/"
        )
    ...
    def scan_new_books(self) -> List[str]:
        ...
        logger.info(
            f"Found {len(book_folders)} books in S3, {len(new_books)} need indexing"
        )
        return new_books
    ...
    def index_book(self, book_name: str, user_id: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Loaded book from S3: {book_data['book_name']} with {book_data['total_chapters']} chapters"
        )
        ...
        logger.info(
            f"Created {len(chunks)} chunks from {book_data['total_chapters']} chapters"
        )
    ...
    def index_all_new_books(self) -> List[Dict[str, Any]]:
        ...
        for book_name in new_books:
            try:
                ...
                results.append({"book_name": book_name, "error": str(e)})

        logger.info(
            f"Indexed {len([r for r in results if 'error' not in r])}/{len(new_books)} books"
        )
        return results
    ...
    def reindex_book(self, book_name: str, user_id: str) -> Dict[str, Any]:
        ...
        self.vector_store.delete_by_filter(
            collection_name=user_id, filter_conditions={"book_name": book_name}
        )
    ...
    def _create_chunks(
        self, text: str, chapter_id: int, chapter_title: str, book_name: str
    ) -> List[Dict[str, Any]]:
        """
